...
void GamePaint(HWND hwnd)
{
	...
	*******************************************************************************/
	...
	hbrushStone = CreateSolidBrush(COLOR_STONE);
	...
	*******************************************************************************/
	///第一个障碍物
	// 将画图需要用的PEN和BRUSH选择到DC中
	hOldBrush = (HBRUSH)SelectObject(hdcmem, hbrushStone);
	hOldPen = (HPEN)SelectObject(hdcmem, hpen);
	...
	// （椭）圆形，使用上面选择的PEN勾勒边框，BRUSH填充
	Rectangle(hdcmem,
		lpStone->x * CELL_PIXEL + rectBoundary.left,
		lpStone->y * CELL_PIXEL + rectBoundary.top,
		(lpStone->x + 1)*CELL_PIXEL + rectBoundary.left,
		(lpStone->y + 1)*CELL_PIXEL + rectBoundary.top);
	...
	* 释放占用的DC等系统资源。
	*******************************************************************************/
	ReleaseDC(hwnd, hdc);///GET之后都要释放DC
}
	...
	LPARAM lParam) // 消息参数，不同的消息有不同的意义，详见MSDN中每个消息的文档
{
	...
	case WM_LBUTTONDOWN:///无用
		OnTimer(hwnd);
		GamePaint(hwnd);
		break;

	case WM_TIMER:///处理计时器消息并重绘窗口
