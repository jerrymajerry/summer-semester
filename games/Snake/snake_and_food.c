...
int CreateStone()
{
	...
	for (j = 0; j < k; j++)
	{
		for (i = 0; i < size; i++)
		{
			...
			{
				// 随机生成障碍物的坐标。
				stone[j].x = rand() % boundary.x;
				stone[j].y = rand() % boundary.y;
			} while ((CoordEqual(posbody, &stone[j]))||(CoordEqual(&food, &stone[j])));// 判断障碍物是否和蛇、食物重叠了，否则重新生成障碍物坐标，直到不重叠。
		}
	}
	return 1;
